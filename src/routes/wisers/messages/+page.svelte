<script lang="ts">
	...
	async function send() {
		...
		sending = true;
		// Find the other user's username from the active conversation
...
<div class="dm-page">
	<div class="dm-sidebar">
		...
		{#if showNewConv}
			<div class="new-conv">
				<input type="text" bind:value={newConvUser} placeholder="Username to message..."
		...
		{:else if conversations.length === 0}
			<div class="dm-empty">No conversations yet. Message a friend to start!</div>
		{:else}
			{#each conversations as conv (conv.id)}
				<button class="conv-item" class:active={activeConv === conv.id} onclick={() => selectConv(conv.id)}>
					...
					<div class="conv-info">
						<div class="conv-name">
							@{conv.other_username}
							{#if conv.my_unread > 0}<span class="unread-badge">{conv.my_unread}</span>{/if}
						</div>
	...
	<div class="dm-chat">
		...
		{:else}
			...
			<div class="chat-input">
				<input type="text" bind:value={newMsg} placeholder="Type a message..."
...
<style>
	...
	.dm-loading, .dm-empty { padding: 24px; text-align: center; font-size: 12px; color: var(--clr-text-muted); }
